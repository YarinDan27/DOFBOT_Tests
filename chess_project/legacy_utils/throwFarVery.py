...
class DOFBOTController:
    ...
    def dramatic_throw_sequence(self):
        """DRAMATIC throwing motion - extend far down, grab, then FAST fold to opposite direction"""
        print("Starting DRAMATIC throw sequence...")
        print("⚠️  This will be a very dynamic motion - ensure area is clear!")
        
        # Step 1: Slowly extend arm all the way down and to the side (open gripper)
        print("Step 1: Slowly extending arm far down to side with open gripper...")
        extend_position = {
            'base': 45,        # Turn to side
            'shoulder': 160,   # Extend shoulder way down
            'elbow': 170,      # Straighten elbow completely
            'wrist_pitch': 180, # Point wrist all the way down
            'wrist_roll': 90,  # Keep wrist straight
            'gripper': 130     # Wide open gripper
        }
        # SLOW extension - 4 seconds to build suspense
        self.move_multiple_servos(extend_position, 4000)
        time.sleep(4.5)  # Wait for full extension
        
        # Step 2: Close gripper to grab something
        print("Step 2: GRABBING! Closing gripper...")
        self.move_servo('gripper', 60, 800)  # Firm grip
        time.sleep(1)
        
        # Step 3: Brief pause for dramatic effect
        print("Step 3: Preparing for EXPLOSIVE motion...")
        time.sleep(0.5)
        
        # Step 4: EXPLOSIVE FAST MOTION - fold to completely opposite direction!
        print("Step 4: MAXIMUM SPEED FOLD TO OPPOSITE SIDE!")
        explosive_fold = {
            'base': 135,       # Swing to complete opposite side (90° swing)
            'shoulder': 20,    # Fold shoulder UP completely (opposite of down)
            'elbow': 20,       # Fold elbow UP tight (opposite of extended)
            'wrist_pitch': 20, # Snap wrist UP (opposite of down)
            'wrist_roll': 90,  # Keep wrist straight
            'gripper': 60      # Keep grip during motion
        }
        # FASTEST possible motion - 200ms!
        self.move_multiple_servos(explosive_fold, 200)
        time.sleep(0.3)
        
        # Step 5: Release at peak of explosive motion
        print("Step 5: RELEASE!")
        self.move_servo('gripper', 140, 100)  # Explosive release
        time.sleep(0.2)
        
        # Step 6: Continue explosive motion for follow-through
        print("Step 6: Follow-through explosion!")
        followthrough = {
            'base': 145,       # Complete the swing
            'shoulder': 10,    # Even higher up
            'elbow': 10,       # Tighter fold
            'wrist_pitch': 10  # Maximum up position
        }
        self.move_multiple_servos(followthrough, 150)  # Even faster follow-through
        time.sleep(0.5)
        
        # Step 7: Slow dramatic return to home
        print("Step 7: Slowly returning to home position...")
        self.home_position(3000)  # Slow, dramatic return
        
        print("DRAMATIC THROW COMPLETE! That was explosive! 💥")
        print("Object should have been launched with maximum force!")
    
    def throw_sequence(self):
        """Original sideways throwing motion"""
        print("Choose throwing style:")
        print("1. Dramatic throw (slow extend, explosive fold)")
        print("2. Sideways throw (classic motion)")
        
        try:
            choice = input("Enter choice (1 or 2), or press Enter for dramatic: ").strip()
            if choice == '2':
                self._sideways_throw()
            else:
                self.dramatic_throw_sequence()
        except:
            self.dramatic_throw_sequence()
    
    def _sideways_throw(self):
        """Original sideways throwing motion - grab, wind back, and throw forward SIDEWAYS"""
        print("Starting sideways throw sequence...")
        ...
        # Step 4: Turn sideways and wind back - SIDEWAYS THROWING POSITION
        print("Step 4: Turning sideways and winding back for sideways throw")
        windup_position = {
            'base': 45,        # Turn sideways (perpendicular to throw direction)
            'shoulder': 90,    # Keep shoulder level (sideways throw)
            'elbow': 45,       # Pull elbow back behind body
            'wrist_pitch': 90, # Keep wrist level for sideways throw
            'wrist_roll': 90,  # Keep wrist straight
        ...
        # Step 5: THE SIDEWAYS THROW! Fast sideways swing motion
        print("Step 5: THROWING! Fast sideways swing motion")
        throw_position = {
            'base': 135,       # Swing base through the throw (sideways motion)
            'shoulder': 90,    # Keep shoulder level throughout throw
            'elbow': 160,      # Extend elbow forward rapidly
            'wrist_pitch': 90  # Keep wrist level for clean release
        }
        # Very fast sideways throwing motion!
        self.move_multiple_servos(throw_position, 300)  # Super fast - 300ms
        time.sleep(0.15)  # Brief pause
        
        # Step 6: Release object during the sideways swing
        print("Step 6: Releasing object during sideways swing!")
        self.move_servo('gripper', 130, 150)  # Quick release during swing
        time.sleep(0.25)
        
        # Step 7: Follow through the sideways motion
        print("Step 7: Follow through sideways swing")
        followthrough_position = {
            'base': 145,       # Complete the sideways rotation
            'shoulder': 90,    # Keep shoulder level
            'elbow': 170,      # Full arm extension
            'wrist_pitch': 90  # Level wrist finish
        }
        self.move_multiple_servos(followthrough_position, 400)
        time.sleep(1)
        ...
        print("Sideways throw sequence complete! That should have been a real throw!")
    ...
    def softball_throw(self):
        ...
        # Step 2: Wind up position - classic pitcher pose AWAY from Jetson
        print("Step 2: Pitcher wind-up position (away from Jetson Nano)")
        windup = {
            'base': 45,        # Turn away from Jetson Nano side
            'shoulder': 45,    # Raise arm up and back
            'elbow': 60,       # Bend elbow up (not down toward Nano)
            'wrist_pitch': 60, # Cock wrist back and up
            'wrist_roll': 90,
        ...
        pitch = {
            'base': 135,       # Swing to opposite side (away from Nano)
            'shoulder': 120,   # Swing arm forward and down
    ...
    def interactive_control(self):
        ...
        print("  throw                    - Execute throwing sequence (with choice)")
        print("  dramatic                 - Execute DRAMATIC throw (slow extend, explosive fold)")
        ...
        while True:
            try:
                ...
                if cmd in ['quit', 'q', 'exit']:
                    ...
                    print("⚠️  DRAMATIC THROW: Slow extend then EXPLOSIVE fold!")
                    print("   This will be very dynamic - ensure area is COMPLETELY clear!")
                    confirm = input("   Continue with dramatic throw? (y/N): ")
                    if confirm.lower() == 'y':
                        self.dramatic_throw_sequence()
                    else:
                        print("   Dramatic throw cancelled")
